...
void TWMCBaseGPU::SetupGPU(size_t gpuId)
{
...
void TWMCBaseGPU::SuggestBatchSize() {
...
std::vector<std::unique_ptr<TaskResults>> TWMCBaseSolver::Compute(const std::vector<std::unique_ptr<TaskData>>& tasks)
{
    ...
    {
        TWMCTaskData *task = static_cast<TWMCTaskData *>(tasks[taskI].get());
...
float TWMCBaseGPU::ApproximateComputationProgress() const
{
